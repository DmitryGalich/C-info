...
### const
- **Определение**: Ключевое слово `const` используется для объявления переменной, значение которой не может быть изменено после инициализации.
- **Пример использования**:
  ```cpp
  const int a = 10;
  ```
- **Время вычисления**: Значение `const` может быть вычислено как во время компиляции, так и во время выполнения.
- **Применение**: `const` можно применять к переменным, параметрам функций, методам и указателям.

### constexpr
- **Определение**: Ключевое слово `constexpr` используется для объявления константы, значение которой должно быть вычислено во время компиляции.
- **Пример использования**:
  ```cpp
  constexpr int b = 20;
  ```
- **Время вычисления**: Значение `constexpr` всегда вычисляется во время компиляции.
- **Применение**: `constexpr` можно применять к переменным, функциям и конструкторам. Функции, объявленные с `constexpr`, должны возвращать значения, которые могут быть вычислены во время компиляции, и все их аргументы также должны быть константами времени компиляции.

### Основные различия
1. **Время вычисления**:
   - `const`: Значение может быть известно как во время компиляции, так и во время выполнения.
   - `constexpr`: Значение всегда известно во время компиляции.

2. **Использование в функциях**:
   - `const`: Не ограничивает функции, может использоваться в любых функциях.
   - `constexpr`: Функции должны быть определены таким образом, чтобы их результат можно было вычислить во время компиляции.

### Пример
```cpp
const int x = 10; // Может быть вычислено во время выполнения
constexpr int y = 20; // Должно быть вычислено во время компиляции

constexpr int square(int n) {
    return n * n;
}

const int result1 = square(x); // Ошибка, так как x не является константой времени компиляции
constexpr int result2 = square(5); // Корректно, так как 5 - константа времени компиляции
```

Надеюсь, это поможет разобраться в различиях между `const` и `constexpr` в C++!
